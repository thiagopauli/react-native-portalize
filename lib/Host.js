...
exports.Host = ({ children, style }) => {
    ...
    React.useEffect(() => {
        ...
        while (queue.length && managerRef.current) {
            ...
            if (action) {
                switch (action.type) {
                    case 'mount':
                        (_a = managerRef.current) === null || _a === void 0 ? void 0 : _a.mount(action.key, action.children, action.style);
                        break;
                    case 'update':
                        (_b = managerRef.current) === null || _b === void 0 ? void 0 : _b.update(action.key, action.children, action.style);
                        break;
                    case 'unmount':
    ...
    const update = (key, children, style) => {
        if (managerRef.current) {
            managerRef.current.update(key, children, style);
        }
        else {
            const op = { type: 'mount', key, children, style };
